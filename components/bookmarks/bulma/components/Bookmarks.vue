<template>
    <core-bookmarks>
        <template v-slot:default="{
            ...
            <div class="bookmarks">
                <span class="control">
                    <a class="tag is-warning icon mr-1" v-on="clearBindings" v-if="hasClear">
                        <fa icon="trash-alt" />
                    </a>
                </span>
                <draggable class="field is-grouped bookmark-items no-scrollbars" v-bind="reorderBindings"
                    v-on="reorderEvents">
                    <span class="control" v-for="bookmark in bookmarks" v-bind="bookmarkBindings(bookmark)"
                        ...
                        <span class="tags has-addons">
                            <a :class="['tag is-bold', { 'is-link': matches($route, bookmark) }]"
                                ...
                                <span class="icon is-small has-text-danger" v-if="bookmark.state">
                                    <fa icon="circle" size="xs" />
                                </span>
                            </a>
                            <a class="tag is-success check" v-on="stickBindings(bookmark)"
                                v-if="!bookmark.sticky && !isExcluded(bookmark)">
                                <span class="icon is-small">
                                    <fa icon="check" />
                                </span>
                            </a>
                            <a class="tag is-delete" v-on="removeBindings(bookmark)"
