...
export default {
  ...
  methods: {
    async fetchData() {
      let params = {start_id: this.selected_person.value}
      this.$axios
          .$get("/api/trees/show", { params: params })
          .then((res) => {
            const generalPerson = res.persons[res.start]
            let children = [];
            generalPerson.own_unions.forEach(item => {
              res.unions[item].children.forEach(child => {
                children.push({
                  id: child.id,
                  name: child.name,
                  title: child.name,
                  ...
                  lastNames:[''],
                  generation: 2,
                  marriage: "1991-08-04T22:15:36.000000Z",
                  xref : 1,
                  sex: child.sex ? child.sex : 'M',
                  timespan: `${child.birth_year ? child.birth_year : child.birthday?.toString().slice(0, 4)}-${child.death_year ? child.death_year : child.deathday?.toString().slice(0, 4)}`,
                  birth: child.birthday ? child.birthday : child.birth_year,
                  death: child.deathday ? child.deathday : child.death_year,
                })
              })
            })
            ...
            //
            //   d3.selectAll("#webtrees-fan-chart-container")
            ...
            // }
            //
            // pedigreeChart.cssFile = "../../assets/css/fan-chart/fan-chart.css";
            // // Draw chart
            let resTest = {
              id:3,
              name: generalPerson.name,
              title: generalPerson.name,
              fullName: "asasas",
              firstName:'Friedrich August',
              lastName: 'a',
              birth:"1890-11-20T22:15:36.000000Z",
              death:"1991-08-04T22:15:36.000000Z",
              marriage: "1991-08-04T22:15:36.000000Z",
              timespan: "1890-1991",
              xref : 1,
              sex: "M",
              generation: 1,
              firstNames:[generalPerson.name],
              lastNames:[''],
              children: children,
            };
            pedigreeChart.draw(resTest);
    ...
    generate() {
      ...
      this.svg = d3.select("#webtrees-fan-chart-container").append("svg")
