  ...
  methods: {
    async setSelected(value) {
      ...
      try {
        ...
        d3.selectAll("#tree").selectAll("svg").remove();
    ...
    generate() {
      ...
      // mark persons
      Object.keys(this.data.persons).forEach(
        (key) => (this.data.persons[key].isUnion = false)
      );
      const margin = {
        top: 20,
        right: 0,
          ...
          const content = `
              <span style='margin-left: 2.5px;'><b>${d.data.name}</b></span><br>
                <table style="margin-top: 2.5px;">
                  <tr><td>born</td><td>${d.data.birthyear || "?"} in ${
            d.data.birthplace || "?"
          }</td></tr>
                  <tr><td>died</td><td>${d.data.deathyear || "?"} in ${
            d.data.deathplace || "?"
          }</td></tr>
                </table>`;
      ...
      this.all_nodes.forEach((n) => {
        ...
        n.data = this.data.persons[n.data.id]
          ? this.data.persons[n.data.id]
          : this.data.unions[n.data.id];
      ...
      const root = this.all_nodes.find((n) => n.data.id === this.data.start);
      root.visible = true;
      root.neighbors = [];
    ...
    uncollapse(d, make_roots) {
      ...
      const extended_neighbors = d.neighbors.filter((n) => n.visible);
      ...
      const collapsed_neighbors = d.neighbors.filter((n) => !n.visible);
    ...
    getParentUnions(node) {
      ...
      if (u_id) {
        const union = this.all_nodes.find((n) => n.data.id === u_id);
    ...
    getParents(node) {
      ...
      if (node.data.isUnion) {
        node.data.partner.forEach((p_id) =>
          parents.push(this.all_nodes.find((n) => n.data.id === p_id))
        );
      } else {
        const parent_unions = this.getParentUnions(node);
    ...
    getPartners(node) {
      ...
      if (node.data.isUnion) {
        node.data.partner.forEach((p_id) =>
          partners.push(this.all_nodes.find((n) => n.data.id === p_id))
        );
      }
      // return other partner of all unions if node argument is a person
      else {
        const own_unions = this.getOwnUnions(node);
    ...
    update(source) {
      ...
      const dag_tree = this.tree(this.dag);
      const nodes = this.dag.descendants();
      const links = this.dag.links();
      // ****************** Nodes section ***************************
      // Update the nodes...
      const node = this.g
        .selectAll("g.node")
      ...
      nodeEnter
        ...
        .attr("r", 1e-6)
        .style("fill", (d) =>
          this.is_extendable(d) ? "lightsteelblue" : "#fff"
        );
      ...
      d3.select("#saveButton").on("click", () => {
        const svgString = this.getSVGString(this.svg.node());
